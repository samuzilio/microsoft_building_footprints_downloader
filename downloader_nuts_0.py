...
target_name = "Italia" # Specify the name of the target country (e.g., "Italia")
target_feature = next((feature for feature in nuts_0["features"] if feature["properties"]["NUTS_NAME"] == target_name), None)
if target_feature:
    target_shape = shapely.geometry.shape(target_feature["geometry"])
    minx, miny, maxx, maxy = target_shape.bounds

# Calculate quad keys for tiles covering the bounding box of the target country
quad_keys = set()
...
# Create a mask GeoDataFrame for the target country
